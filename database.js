...
export function saveMenuItems(menuItems) {
  db.transaction((tx) => {
    ...
    menuItems.forEach((item, index) => {
      if (index != 0)
        items += ","
      items += `(${item.id}, '${item.id}', '${item.title}', ${item.price}, '${item.category}')`
    });
    tx.executeSql(`insert into menuitems(id,uuid,title,price,category) values${items};`, [], () => { console.log("done") }, (error) => { console.log(error) });
  });
}
...
export async function filterByQueryAndCategories(query, activeCategories) {
  ...
  let sqlQuery=`SELECT * FROM menuitems WHERE ${query && `title LIKE '%${query}%'`} ${query && cat && "AND"} ${cat && `category IN (${cat})`};`
  return new Promise((resolve, reject) => {
    db.transaction(tx => {
      tx.executeSql(
        sqlQuery,
